...
public class GetLoc extends Service implements LocationListener {
	...
	@Override
	public int onStartCommand(Intent intent, int flags, int startid) {	// 실행되면서 설정, startService()를 호출하면 실행된다.
		// TODO Auto-generated method stub
		Context = this;
		Log.e("tag", "on start");
	...
	public Location getLocation(Context Context) {
		...
		try {
			...
			isGPSEnabled = locationManager
					.isProviderEnabled(LocationManager.GPS_PROVIDER);// 현재 위치를 GPS가 알아냄
				...
				if (isGPSEnabled) {
					if (location == null) {
						locationManager.requestLocationUpdates(
								...
								MIN_TIME_BW_UPDATES,
								MIN_DISTANCE_CHANGE_FOR_UPDATES, this);	// location을 얼마마다 알려줄 것인가 설정
