package com.example.walkpathexample2;
...
public class MainActivity extends FragmentActivity implements OnMapReadyCallback {
	...
	public void drawmap(double latid, double longid) {
		...
		map.animateCamera(CameraUpdateFactory.newLatLngZoom(my, 10.0f), 2000, null);		// 줄을 긋는다.
		pLati = latid;
		plongi = longid;
	}
	...
	protected void onResume() {
		super.onResume();
		registerReceiver(receiver, new IntentFilter("com.example.map"));		// 주파수 맞추는 준비, IntentFilter 객체를 통해서 걸러낸다.
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mapFragment.getMapAsync(this);		// 맵을 그리는 일은 시간이 많이 걸리기 때문에 ssync객체를 이용한다.
//
